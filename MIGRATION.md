...
`dapr-sdk-workflows` is still compiled for **Java 17** and runs on Java 17 or later, and the runtime
still defaults to a cached thread pool.

One behaviour changed: an executor you supply with `WorkflowRuntimeBuilder.withExecutorService(...)`
is no longer shut down when the runtime closes. The runtime now shuts down only an executor it
created for itself. If you relied on the runtime to shut down an executor you passed in, shut it
down yourself.
...
- [ ] Shut down any executor you pass to `withExecutorService(...)` yourself
