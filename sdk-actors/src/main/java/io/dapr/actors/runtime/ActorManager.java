...
class ActorManager<T extends AbstractActor> {
     ...
     * @param request      Parameters for the reminder.
     * @return Asynchronous void response.
     */
    Mono<Void> invokeReminder(ActorId actorId, String reminderName, String request) {
        ...
        try {
            ActorReminderParams params = this.runtimeContext.getActorSerializer().deserialize(request, ActorReminderParams.class);

            return invoke(
                    actorId,
                    ActorMethodContext.CreateForReminder(reminderName),
                    actor -> doReminderInvokation((Remindable) actor, reminderName, params))
                    .then();
        } catch (Exception e) {
            return Mono.error(e);
        }
