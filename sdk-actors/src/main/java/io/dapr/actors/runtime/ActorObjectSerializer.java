...
public class ActorObjectSerializer extends ObjectSerializer {
    ...
    private static Duration extractDurationOrNull(JsonNode node, String name) {
        ...
        return DurationUtils.convertDurationWithRepetitionFromDapr(valueNode.asText()).getDuration();
    }

    /**
     * Extracts repetition or null.
     *
     * @param node Node that contains the attribute.
     * @param name Attribute name.
     * @return Parsed duration or null.
     */
    private static Integer extractRepetitionOrNull(JsonNode node, String name) {
        JsonNode valueNode = node.get(name);
        if (valueNode == null) {
            return null;
        }

        return DurationUtils.convertDurationWithRepetitionFromDapr(valueNode.asText()).getRepetitions()
                .orElse(null);
    }
    ...
    private byte[] serialize(ActorReminderParams reminder) throws IOException {
        try (ByteArrayOutputStream writer = new ByteArrayOutputStream()) {
            ...
            generator.writeStringField("period", DurationUtils.convertDurationWithRepetitionToISO8601Format(new DurationUtils.RepeatedDuration(reminder.getPeriod(), reminder.getRepetitions())));
    ...
    private ActorReminderParams deserializeActorReminder(byte[] value) throws IOException {
        ...
        Duration dueTime = extractDurationOrNull(node, "dueTime");
        Duration period = extractDurationOrNull(node, "period");
        Integer repetition = extractRepetitionOrNull(node, "period");
        byte[] data = node.get("data") != null ? node.get("data").binaryValue() : null;

        return new ActorReminderParams(data, dueTime, period, repetition);
    }
}
