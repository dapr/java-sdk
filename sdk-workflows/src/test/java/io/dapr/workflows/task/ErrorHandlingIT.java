...
public class ErrorHandlingIT extends IntegrationTestBase {
  ...
  void activityException(boolean handleException) throws TimeoutException {
    ...
    try (worker; client) {
      ...
      if (handleException) {
        ...
        assertEquals(TaskFailedException.class.getName(), details.getErrorType());
        assertNotNull(details.getStackTrace());
  ...
  public void retryActivityFailures(int maxNumberOfAttempts) throws TimeoutException {
    ...
    this.retryOnFailuresCoreTest(maxNumberOfAttempts, expectedTaskCount, ctx -> {
      WorkflowTaskRetryPolicy retryPolicy = getCommonRetryPolicy(maxNumberOfAttempts);
      ctx.callActivity(
              "BustedActivity",
              null,
              WorkflowTaskOptions.withRetryPolicy(retryPolicy)).await();
    });
  }
  ...
  public void retryActivityFailuresWithCustomLogic(int maxNumberOfAttempts) throws TimeoutException {
    ...
    this.retryOnFailuresCoreTest(maxNumberOfAttempts, maxNumberOfAttempts, ctx -> {
      WorkflowTaskRetryHandler retryHandler = getCommonRetryHandler(retryHandlerCalls, maxNumberOfAttempts);
  ...
  void subOrchestrationException(boolean handleException) throws TimeoutException {
    ...
    DurableTaskGrpcWorker worker = this.createWorkerBuilder()
            .addOrchestrator(orchestratorName, ctx -> {
              try {
                String result = ctx.callChildWorkflow(subOrchestratorName, "", String.class).await();
                ctx.complete(result);
              } catch (TaskFailedException ex) {
                if (handleException) {
                  ctx.complete("handled");
  ...
  public void retrySubOrchestrationFailuresWithCustomLogic(int maxNumberOfAttempts) throws TimeoutException {
    ...
    this.retryOnFailuresCoreTest(maxNumberOfAttempts, maxNumberOfAttempts, ctx -> {
      WorkflowTaskRetryHandler retryHandler = getCommonRetryHandler(retryHandlerCalls, maxNumberOfAttempts);
      WorkflowTaskOptions options = WorkflowTaskOptions.withRetryHandler(retryHandler);
      ctx.callChildWorkflow("BustedSubOrchestrator", null, null, options).await();
    });
  ...
  private static WorkflowTaskRetryPolicy getCommonRetryPolicy(int maxNumberOfAttempts) {
    // Include a small delay between each retry to exercise the implicit timer path
    return new WorkflowTaskRetryPolicy(maxNumberOfAttempts, Duration.ofMillis(1));
  }

  private static WorkflowTaskRetryHandler getCommonRetryHandler(AtomicInteger handlerInvocationCounter, int maxNumberOfAttempts) {
    return ctx -> {
      // Retry handlers get executed on the orchestrator thread and go through replay
      if (!ctx.getWorkflowContext().isReplaying()) {
   ...
   */
  private WorkflowFailureDetails retryOnFailuresCoreTest(
