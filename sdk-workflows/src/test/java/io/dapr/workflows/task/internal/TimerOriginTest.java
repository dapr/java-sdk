...
class TimerOriginTest {
  ...
  void test3_activityRetryTimerSetsActivityRetryOrigin() {
    WorkflowTaskRetryPolicy policy = new WorkflowTaskRetryPolicy(2, Duration.ofSeconds(1));
    WorkflowTaskOptions options = WorkflowTaskOptions.withRetryPolicy(policy);
  ...
  void test4_activityRetryTaskExecutionIdStable() {
    WorkflowTaskRetryPolicy policy = new WorkflowTaskRetryPolicy(3, Duration.ofSeconds(1));
    WorkflowTaskOptions options = WorkflowTaskOptions.withRetryPolicy(policy);
  ...
  void test5_childWorkflowRetryTimerSetsChildWorkflowRetryOrigin() {
    ...
    TaskOrchestration orchestration = ctx -> {
      try {
        ctx.callChildWorkflow("Child", null, childInstanceId, options, String.class).await();
      } catch (TaskFailedException e) {
        // swallow
      }
    };
  ...
  void test6_childWorkflowRetryInstanceIdStaysOnFirstChild() {
    ...
    TaskOrchestration orchestration = ctx -> {
      try {
        ctx.callChildWorkflow("Child", null, firstChildInstanceId, options, String.class).await();
      } catch (TaskFailedException e) {
        // swallow
      }
    };
  ...
  void test11_prePatchReplayIndefiniteWaitThenCallChildWorkflow() {
    TaskOrchestration orchestration = ctx -> {
      ctx.waitForExternalEvent("myEvent", Duration.ofSeconds(-1), String.class).await();
      ctx.callChildWorkflow("Child", null, "child-1", null, String.class).await();
    };
