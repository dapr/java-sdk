 ...
 * {@link io.dapr.quarkus.langchain4j.agent.workflow.AgentRunWorkflow} and set the context.
...
public class DaprToolCallInterceptor {
  ...
  private String tryLazyActivate(String toolMethodName) {
    try {
      ...
      LOG.infof("[AgentRun:%s] Lazy activation triggered by first tool call: %s",
          agentRunId, toolMethodName);
      return agentRunId;
    } catch (Exception e) {
